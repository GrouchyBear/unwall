...
exports.createUrl = options => {
    ...
    return `https://source.unsplash.com/category/buildings/${options.width}x${options.height}`;
};

// return promise with filename
exports.download = (options, url, reporter) => {
    const dir = (options.dir === '.') ? process.cwd() : options.dir;
    const rand = Math.random().toString(36).slice(4, 19);
    const uniqueName = path.join(dir, `wallpaper-${rand}.png`);
